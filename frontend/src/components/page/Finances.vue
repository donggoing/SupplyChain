    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 融资记录
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            ...
            >
                ...
                <el-table-column prop="amount" label="融资Token数" align="center"></el-table-column>
                <el-table-column prop="applyRate" label="申请费率/10000" align="center"></el-table-column>
                ...
                <el-table-column label="银行接收" align="center">
                    <template slot-scope="scope" align="center">
                        <el-tag
...
export default {
    ...
    methods: {
        getData() {
            this.$http.get('/normal/allAp4Finances').then(
                res=>{
        ...
        handleRefuse(index, row) {
            this.$confirm('确定拒绝吗？', '提示', {
                type: 'warning'
                    ...
                    this.$http.patch('/bank/ap4Finance',{index:row.index,acceptOrNot:false}).then(res=>{
                        if(res.data&&res.data.success)                        
                            this.$message.success('成功拒绝');
                        else this.$message.error(res.data.detail?res.data.detail:"拒绝失败，请稍后再试");
                        this.getData();
                    })
                })
        ...
        handleAccept(index, row) {
            this.$confirm('确定接受吗？', '提示', {
                ...
                .then(
                    () => {
                        this.$http.patch('/bank/ap4Finance',{index:row.index,acceptOrNot:true}).then(res=>{
                            if(res.data&&res.data.success)                        
